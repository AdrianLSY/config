...
This project has a knowledge graph at `graphify-out/`. **Note:** after the tier restructure the tracked `graph.json` paths are stale (they reference the old flat layout) until you run `graphify update .`.

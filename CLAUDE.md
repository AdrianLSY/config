...
Branches: **`main` is the canonical working branch (macOS).** As of 2026-07-01 the old `macos` branch was folded into `main` and retired (along with the earlier "lean cross-platform base" model), so macOS work now happens directly on `main`. `cachyos-home` and `cachyos-work` are the separate Linux machine branches — **never merge `main` into them wholesale**, since `main` carries macOS-only config (`aerospace/`, `linearmouse/`) and `defaults` tweaks that break on Linux; port only the cross-platform bits by hand.
